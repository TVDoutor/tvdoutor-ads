...
-- 1) PROPOSALS: parâmetros e métricas (corridos + úteis) - only if table exists
DO $$
BEGIN
...
-- 2) PROPOSAL_SCREENS: overrides por tela - only if table exists
DO $$
BEGIN
...
-- 4) VIEWS - only if required tables exist
DO $$
BEGIN
...
-- 6) TRIGGERS - only if tables exist
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'proposals' AND table_schema = 'public') THEN
    EXECUTE 'CREATE OR REPLACE FUNCTION public.trg_proposals_recalc()
    ...
    EXECUTE 'DROP TRIGGER IF EXISTS proposals_recalc ON public.proposals';
    EXECUTE 'CREATE TRIGGER proposals_recalc
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'proposal_screens' AND table_schema = 'public') THEN
    EXECUTE 'CREATE OR REPLACE FUNCTION public.trg_proposal_screens_recalc()
    RETURNS trigger LANGUAGE plpgsql AS $func$
    ...
    EXECUTE 'DROP TRIGGER IF EXISTS proposal_screens_recalc ON public.proposal_screens';
    EXECUTE 'CREATE TRIGGER proposal_screens_recalc
