-- 1. Melhorias na tabela agencia_projetos - apenas se tabela existir
DO $$
BEGIN
...
-- 2. Nova tabela para equipe do projeto - apenas se agencia_projetos existir
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'agencia_projetos' AND table_schema = 'public') THEN
...
-- 3. Nova tabela para marcos do projeto - apenas se agencia_projetos existir
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'agencia_projetos' AND table_schema = 'public') THEN
...
-- 4. Views para facilitar consultas - apenas se tabelas existirem
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'agencia_projetos' AND table_schema = 'public') AND
       ...
       EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'agencias' AND table_schema = 'public') THEN
        ...
        EXECUTE 'CREATE OR REPLACE VIEW vw_projetos_disponiveis AS 
        SELECT 
          ap.id,
          ap.nome_projeto,
          ap.status_projeto,
...
-- 6. RLS Policies - apenas se tabelas existirem
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'agencia_projeto_equipe' AND table_schema = 'public') THEN
        ...
        -- Dropping old policies before creating new ones
        DROP POLICY IF EXISTS projeto_equipe_policy ON agencia_projeto_equipe;

        CREATE POLICY projeto_equipe_policy ON agencia_projeto_equipe
          FOR ALL USING (
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'agencia_projeto_marcos' AND table_schema = 'public') THEN
        ...
        CREATE POLICY projeto_marcos_policy ON agencia_projeto_marcos
          FOR ALL USING (
            auth.uid() IS NOT NULL AND (
              is_super_admin() OR
...
-- 7. Índices para performance - apenas se tabelas existirem
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'agencia_projetos' AND table_schema = 'public') THEN
