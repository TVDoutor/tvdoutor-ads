-- ============================================
-- MIGRAÇÃO PARA CRIAR SISTEMA DE CAMPANHAS
...
-- Add updated_at trigger for campaigns (only if function doesn't exist)
...
SELECT 'campaigns' as table_name, COUNT(*) as exists FROM information_schema.tables WHERE table_name = 'campaigns' AND table_schema = 'public'
UNION ALL
SELECT 'campaign_screens' as table_name, COUNT(*) as exists FROM information_schema.tables WHERE table_name = 'campaign_screens' AND table_schema = 'public';

-- Verificar se as políticas RLS foram criadas
SELECT schemaname, tablename, policyname FROM pg_policies WHERE tablename IN ('campaigns', 'campaign_screens') ORDER BY tablename, policyname;
