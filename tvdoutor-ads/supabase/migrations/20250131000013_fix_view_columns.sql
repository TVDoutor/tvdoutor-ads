-- ===============================
-- FIX VIEW COLUMNS
-- Date: 2025-01-31
-- Corrigir colunas da view v_screens_enriched
-- ===============================

BEGIN;

-- Verificar se a tabela screens tem as colunas necessárias
DO $$
BEGIN
  -- Verificar se a coluna 'class' existe na tabela screens
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'screens' 
    AND column_name = 'class'
    AND table_schema = 'public'
  ) THEN
    -- Adicionar a coluna 'class' se não existir
    ALTER TABLE public.screens ADD COLUMN class text;
    RAISE NOTICE 'Coluna class adicionada à tabela screens';
  END IF;

  -- Verificar se a coluna 'address_raw' existe na tabela screens
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'screens' 
    AND column_name = 'address_raw'
    AND table_schema = 'public'
  ) THEN
    -- Adicionar a coluna 'address_raw' se não existir
    ALTER TABLE public.screens ADD COLUMN address_raw text;
    RAISE NOTICE 'Coluna address_raw adicionada à tabela screens';
  END IF;

  -- Verificar se a coluna 'specialty' existe na tabela screens
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'screens' 
    AND column_name = 'specialty'
    AND table_schema = 'public'
  ) THEN
    -- Adicionar a coluna 'specialty' se não existir
    ALTER TABLE public.screens ADD COLUMN specialty text[];
    RAISE NOTICE 'Coluna specialty adicionada à tabela screens';
  END IF;

  -- Verificar se a coluna 'board_format' existe na tabela screens
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'screens' 
    AND column_name = 'board_format'
    AND table_schema = 'public'
  ) THEN
    -- Adicionar a coluna 'board_format' se não existir
    ALTER TABLE public.screens ADD COLUMN board_format text;
    RAISE NOTICE 'Coluna board_format adicionada à tabela screens';
  END IF;

  -- Verificar se a coluna 'category' existe na tabela screens
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'screens' 
    AND column_name = 'category'
    AND table_schema = 'public'
  ) THEN
    -- Adicionar a coluna 'category' se não existir
    ALTER TABLE public.screens ADD COLUMN category text;
    RAISE NOTICE 'Coluna category adicionada à tabela screens';
  END IF;
END $$;

-- Recriar a view com todas as colunas necessárias
DROP VIEW IF EXISTS public.v_screens_enriched;

CREATE OR REPLACE VIEW public.v_screens_enriched AS
SELECT
  -- Campos principais da tela
  sc.id,
  sc.code,
  sc.name,
  sc.display_name,
  sc.city,
  sc.state,
  sc.cep,
  COALESCE(sc.address_raw, '') AS address,
  sc.lat,
  sc.lng,
  sc.geom,
  sc.active,
  COALESCE(sc.class, 'ND') AS class,
  -- Garantir que specialty seja sempre um array
  COALESCE(sc.specialty, ARRAY[]::text[]) AS specialty,
  COALESCE(sc.board_format, 'LED') AS board_format,
  COALESCE(sc.category, 'Outdoor') AS category,
  
  -- Dados de rates (se existirem)
  sr.standard_rate_month,
  sr.selling_rate_month,
  sr.spots_per_hour,
  sr.spot_duration_secs,
  
  -- Dados do venue (se existir)
  v.name AS venue_name,
  v.google_formatted_address AS venue_address,
  v.country AS venue_country,
  v.state AS venue_state,
  v.district AS venue_district,
  
  -- Campo rede fixo baseado no código da tela
  CASE 
    WHEN sc.code LIKE 'P2%' THEN 'TV Doutor'
    WHEN sc.code LIKE 'P1%' THEN 'LG'
    WHEN sc.code LIKE 'P3%' THEN 'Amil'
    ELSE 'TV Doutor'
  END AS rede,
  
  -- Campos de staging para compatibilidade
  sc.name AS staging_nome_ponto,
  NULL::integer AS staging_audiencia,
  -- Garantir que staging_especialidades seja uma string
  CASE 
    WHEN sc.specialty IS NOT NULL AND array_length(sc.specialty, 1) > 0 
    THEN array_to_string(sc.specialty, ',')
    ELSE NULL
  END AS staging_especialidades,
  NULL::text AS staging_tipo_venue,
  NULL::text AS staging_subtipo,
  sc.category AS staging_categoria,
  
  -- Timestamps
  sc.created_at,
  sc.updated_at
  
FROM public.screens AS sc
LEFT JOIN public.screen_rates sr ON sr.screen_id = sc.id
LEFT JOIN public.venues v ON v.id = sc.venue_id
WHERE sc.active = true
ORDER BY sc.code;

-- Comentário da view
COMMENT ON VIEW public.v_screens_enriched IS 'View enriquecida com todas as colunas necessárias para o frontend';

-- Permitir acesso à view para usuários autenticados
GRANT SELECT ON public.v_screens_enriched TO authenticated;

-- Verificar se a view está funcionando
DO $$
BEGIN
  -- Testar se a view pode ser consultada
  PERFORM * FROM public.v_screens_enriched LIMIT 1;
  RAISE NOTICE 'View v_screens_enriched criada com sucesso!';
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Erro ao criar view v_screens_enriched: %', SQLERRM;
END $$;

COMMIT;
