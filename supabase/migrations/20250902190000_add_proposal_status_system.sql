...
-- Adicionar campos para controle de envio de email
DO $$
BEGIN
...
-- Aplicar trigger apenas se a coluna status_updated_at existir
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
...
CREATE INDEX IF NOT EXISTS idx_proposals_user_status ON public.proposals(created_by, status);

-- Índice para ordenação por data de atualização de status
CREATE INDEX IF NOT EXISTS idx_proposals_status_updated ON public.proposals(status_updated_at DESC);

-- ===============================
-- 6. VIEW PARA ESTATÍSTICAS DE PROPOSTAS
-- ===============================

-- View para estatísticas rápidas por status
CREATE OR REPLACE VIEW public.proposal_status_stats AS
SELECT 
    status,
    COUNT(*) as total,
...
-- Função para alterar status de proposta com validação
...
'Status das propostas: rascunho -> enviada -> em_analise -> aceita/rejeitada';

COMMENT ON COLUMN public.proposals.status IS 
'Status atual da proposta seguindo fluxo de aprovação';

COMMENT ON COLUMN public.proposals.email_sent_at IS 
'Timestamp de quando o email da proposta foi enviado ao cliente';

COMMENT ON COLUMN public.proposals.notes IS 
'Observações e notas sobre a proposta';

COMMENT ON COLUMN public.proposals.status_updated_at IS 
'Timestamp da última alteração de status';

COMMENT ON FUNCTION public.update_proposal_status(bigint, public.proposal_status, text) IS 
'Função segura para alterar status de proposta com validação de regras de negócio';

COMMENT ON VIEW public.proposal_status_stats IS 
