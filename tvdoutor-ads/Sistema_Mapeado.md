...
3. **Inventário de Telas (Screens)**
   ...
   - Exportação de inventário para Excel
   - Normalização automática de especialidades médicas
   - **NOVO (v1.4):** Campos por tela: **ambiente** (tipo/característica do ambiente), **audiências segmentadas** (pacientes, local, HCP, médica), **aceita_convenio**, **audience_monthly** (audiência mensal editável); utilizados em inventário, venues, relatórios e propostas
...
12. **Profissionais da Saúde (NOVO)**
    - Cadastro de profissionais (nome, tipo, registro, email, telefone)
    - Vínculos profissional ↔ venue (unidades/locais)
    - Cargo por unidade, ativo/inativo
    - Listagem com busca e filtros
    - Integração com venues e especialidades (quando disponível)

13. **Farmácias (NOVO)**
    - Cadastro de farmácias (razão social, fantasia, CNPJ, endereço completo)
    - Geolocalização (lat/lng) e endereço formatado (Google)
    - Importação em massa e exportação para Excel
    - Listagem com filtros por cidade, UF, grupo
    - Mapeamento de status (pendente mapeamento, com coordenadas)

14. **Monitor de Usuários (User Management)**
    - Página exclusiva para **super_admin** (`/user-management`)
    - Dashboard de sessões de usuários online (quando ativo)
    - Histórico de sessões e atividades
    - Diferente de `/users` (gestão de usuários/roles para admin)
...
- **NOVO:** `Pharmacies`: Gestão de farmácias (cadastro, import/export Excel, geolocalização)
- **NOVO:** `ProfissionaisSaude`: Gestão de profissionais da saúde e vínculos com venues
- **NOVO:** `UserManagement`: Monitor de usuários e sessões (super_admin apenas)
...
12. **NOVO:** `tvd-player-status`: Status dos players TVDoutor
13. **NOVO:** `tvd-sync-players`: Sincronização de players
14. **NOVO:** `tvd-verify-sync-player`: Verificação de sincronização de player

**Scripts de administração:**
- `scripts/create-admin-user-simple.js`: Criação de usuário admin via Node (Supabase Auth + profiles + user_roles). Uso: `node scripts/create-admin-user-simple.js` (requer `VITE_SUPABASE_URL` e `SUPABASE_SERVICE_ROLE_KEY`). Edge Function `create-admin-user` disponível como alternativa via API.
...
### 2.5.10 Profissionais da Saúde

**Descrição**: Módulo de gestão do corpo clínico (profissionais da saúde) com vínculos a venues (unidades).

**Estrutura de Dados:**
- **profissionais_saude**: id, nome, tipo_profissional, tipo_registro, registro_profissional, email, telefone, ativo, created_at, updated_at, created_by
- **profissional_venue**: vínculo N:N entre profissional e venue (cargo_na_unidade)
- **view_detalhes_profissionais**: view que agrega profissional + venue + especialidades (quando specialties existir)

**Funcionalidades:**
- CRUD de profissionais (nome, tipo, registro profissional, email, telefone)
- Vínculos com venues (unidades) e cargo na unidade
- Listagem com busca por nome, registro ou tipo
- Diálogos: formulário de cadastro/edição e gestão de vínculos
- Badges de status (ativo/inativo) e popover de vínculos
- Permissões: usuários autenticados podem ler/escrever (RLS configurável)

**Rotas e componentes:**
- Rota: `/profissionais-saude`
- Página: `ProfissionaisSaude`
- Componentes: `ProfissionalFormDialog`, `ProfissionalVinculoDialog`, `VinculosPopover`
- Hooks: `useProfissionaisSaude`, `useDeleteProfissional`

### 2.5.11 Farmácias

**Descrição**: Cadastro e gestão de farmácias com endereço completo, geolocalização e import/export em Excel.

**Funcionalidades:**
- Cadastro de farmácias: nome, fantasia, CNPJ, endereço (tipo logradouro, número, complemento, bairro, cidade, UF, CEP), grupo, lat/lng
- Campos opcionais: `nome` (razão social), `google_formatted_address` (endereço formatado por geocoding)
- Importação em massa e exportação para Excel (ExcelJS)
- Bulk upsert via serviço e API Supabase
- Listagem com filtros (cidade, UF, grupo, busca), ordenação e paginação
- Indicador de “pendente mapeamento” para registros sem coordenadas
- Integração com Google Maps/geocoding para preenchimento de endereço e coordenadas

**Rotas e componentes:**
- Rota: `/farmacias`
- Página: `Pharmacies`
- Serviço: `pharmacy-service` (fetchAllPharmacies, createPharmacy, updatePharmacyRecord, deletePharmacyRecord, bulkUpsertPharmacies)
- Tabela no banco: `farmacias` (migrações: nome, google_formatted_address)

### 2.5.12 Venue–Farmácia (distância e RPCs) – v1.4.0

**Descrição**: View e funções para relacionar venues a farmácias por distância (Haversine), permitindo filtrar telas/venues por “farmácia a até X km” e relatórios por especialidade e raio.

**Estrutura:**
- **haversine_km(lat1, lon1, lat2, lon2)**: função SQL que retorna distância em km entre dois pontos.
- **mv_venue_farmacia_distancia** (view): pares (venue_id, farmacia_id) com distância em km, limitada a 30 km; usa `venues` e `farmacias` com coordenadas.
- **get_venue_ids_with_pharmacy_in_radius(radius_km)**: retorna `venue_id` que possuem ao menos uma farmácia dentro do raio informado.
- **get_farmacia_ids_in_radius(radius_km)**: retorna `farmacia_id` que estão a até `radius_km` de algum venue (uso no mapa para exibir apenas farmácias no raio).
- **get_pharmacy_count_by_specialty_and_radius(p_specialty, p_radius_km)**: retorna JSON com contagem e lista de farmácias no raio de venues que têm a especialidade informada.
- **get_venues_by_pharmacy_radius_summary(radii_km)**: para cada raio no array (ex.: 1,2,3,4,5 km), retorna quantidade de venues e de telas ativas com farmácia no raio.

**Uso:** Filtros no mapa (mostrar só farmácias no raio), relatórios por especialidade/raio, e elaboração de propostas com critério de proximidade venue–farmácia.
...
A tabela evoluiu ao longo das migrações. Além dos campos clássicos (id, name, venue_id, address, city, state, lat/lng, class, type, status, etc.), existem:

- **Campos de staging/audiência:** `code`, `display_name`, `address_raw`, `cep`, `geom`, `active`, `specialty` (array), `board_format`, `category`, `audience_monthly` (migração 20260204), `venue_type_parent`, `venue_type_child`, `venue_type_grandchildren`.
- **Tabela auxiliar:** `screen_rates` (standard_rate_month, selling_rate_month, spots_per_hour, spot_duration_secs) vinculada por `screen_id`.
- **Campos adicionais (v1.4 – migração 20260212):**
  ...
  - `aceita_convenio` (boolean): se o local aceita convênio.

Utilizados em inventário, detalhes de venue, relatórios e elaboração de propostas.
...
#### Profissionais da Saúde (NOVO - v1.3.0)

**profissionais_saude** (Profissionais da Saúde)
```sql
...
**profissional_venue** (Vínculos Profissional ↔ Venue)
```sql
...
```
- View `view_detalhes_profissionais` agrega profissional + venue + especialidades (quando existir tabela specialties).

#### Farmácias (NOVO - v1.3.0)

**farmacias** (Cadastro de Farmácias)
- Campos: id, nome, fantasia, cnpj, tipo_logradouro, endereco, numero, complemento, bairro, cidade, uf, cep, grupo, lat, lng, google_formatted_address (opcional), created_at, updated_at (conforme migrações).
- Utilizada pela página Pharmacies para listagem, CRUD, import/export Excel e geolocalização.
...
View atualizada nas migrações 20260204 e 20260212. Faz `screens` (sc) LEFT JOIN `screen_rates` (sr), expondo:

- Identificação e local: id, code, name, display_name, city, state, cep, address (address_raw), lat, lng, geom, active, class, specialty, board_format, category, rede ('TV Doutor').
- Rates: standard_rate_month, selling_rate_month, spots_per_hour, spot_duration_secs.
- Staging: staging_nome_ponto, staging_audiencia (audience_monthly), staging_especialidades, staging_tipo_venue, staging_subtipo, staging_categoria, venue_type_grandchildren, address_raw.
- **Campos v1.4:** ambiente, audiencia_pacientes, audiencia_local, audiencia_hcp, audiencia_medica, aceita_convenio.
- Filtro: apenas telas com lat e lng não nulos.

Grant SELECT para authenticated e anon.
...
### 12/02/2026 - v1.4.0
- **Atualização**: Telas (screens) e venue–farmácia
- **Escopo**:
  - ✅ **screens**: novos campos `ambiente`, `audiencia_pacientes`, `audiencia_local`, `audiencia_hcp`, `audiencia_medica`, `aceita_convenio` (migração 20260212000002); utilizados em inventário, venues, relatórios e propostas
  - ✅ **v_screens_enriched**: atualizada para expor os novos campos e staging (venue_type_*, audience_monthly)
  - ✅ **Venue–Farmácia**: view `mv_venue_farmacia_distancia` (pares venue–farmácia com distância até 30 km), função `haversine_km`, RPCs `get_venue_ids_with_pharmacy_in_radius`, `get_farmacia_ids_in_radius`, `get_pharmacy_count_by_specialty_and_radius`, `get_venues_by_pharmacy_radius_summary` (migrações 20260212000000, 20260212000001)
- **Observações**: Filtros por “farmácia no raio” no mapa e relatórios por especialidade/raio; documentação alinhada às migrações recentes.

### 09/02/2026 - v1.3.0
- **Atualização**: Documentação e mapeamento com novas funcionalidades
- **Escopo**:
  - ✅ Módulo **Profissionais da Saúde** (`/profissionais-saude`): cadastro, vínculos com venues, listagem e filtros
  - ✅ Módulo **Farmácias** (`/farmacias`): cadastro, import/export Excel, geolocalização, tabela `farmacias`
  - ✅ Página **User Management** (`/user-management`): monitor de usuários/sessões (super_admin)
  - ✅ Edge Functions: `tvd-player-status`, `tvd-sync-players`, `tvd-verify-sync-player`
  - ✅ Script `create-admin-user-simple.js` e referência à Edge Function `create-admin-user`
  - ✅ Tabelas documentadas: `profissionais_saude`, `profissional_venue`, `farmacias`
  - ✅ Novas páginas e componentes listados no mapeamento
- **Observações**: Versão do sistema 1.3.0; 14 Edge Functions listadas; rotas e módulos alinhados ao App e Sidebar.
...
- `FUNCIONALIDADES.md` - Todas as funcionalidades e como devem funcionar
