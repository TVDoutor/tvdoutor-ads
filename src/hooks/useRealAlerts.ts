  ...
  return useQuery({
    ...
    queryFn: async (): Promise<RealAlertGroup[]> => {
      ...
      try {
        ...
        // Equipamentos offline > 24h (tvd_player_status, cache GraphQL)
        const playerOfflineAlerts = await getPlayerOffline24hAlerts();
        ...
        const alertGroups: RealAlertGroup[] = [
          ...
          },
          {
            ...
            name: 'Equipamentos offline > 24h',
            count: playerOfflineAlerts.length,
            criticalCount: playerOfflineAlerts.filter(a => a.severity === 'critical').length,
            alerts: playerOfflineAlerts
          }
        ];
