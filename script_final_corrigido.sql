-- Script final corrigido - verifica estrutura e cria tabelas corretas
-- Execute este script no Supabase SQL Editor

-- 1. Primeiro, vamos verificar a estrutura atual da tabela agencia_deals
...
-- 2. Se a tabela agencia_deals não tiver as colunas corretas, vamos recriá-la
DROP TABLE IF EXISTS public.agencia_deals CASCADE;

-- 3. Criar tabela agencia_deals com a estrutura correta
CREATE TABLE public.agencia_deals (
...
-- 4. Verificar se a tabela agencia_projetos existe e tem a estrutura correta
SELECT column_name, data_type, is_nullable 
FROM information_schema.columns 
...
-- 5. Se a tabela agencia_projetos não tiver as colunas corretas, vamos recriá-la
DROP TABLE IF EXISTS public.agencia_projetos CASCADE;

-- 6. Criar tabela agencia_projetos com a estrutura correta
CREATE TABLE public.agencia_projetos (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- 8. Criar políticas para agencia_deals
...
-- 9. Criar políticas para agencia_projetos
...
-- 11. Verificar se o deal foi criado
SELECT 
    ad.id,
    ad.nome_deal,
    ad.status,
...
-- 13. Verificar se o projeto foi criado
SELECT 
    ap.id,
    ap.nome_projeto,
    ap.status_projeto,
...
-- 14. Verificar todas as tabelas criadas
SELECT 
    table_name,
    table_type
