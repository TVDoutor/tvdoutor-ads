...
-- Adicionar created_by se não existir (com segurança)
DO $$
BEGIN
...
-- Aplicar trigger apenas se a coluna updated_at existir
DO $$
BEGIN
...
-- Habilitar RLS na tabela proposals (se ainda não estiver)
...
-- Habilitar RLS na tabela proposal_screens
...
-- Aplicar trigger apenas se a coluna created_by existir
DO $$
BEGIN
...
COMMENT ON POLICY "proposals_insert_authenticated" ON public.proposals IS 
'Permite que usuários autenticados criem propostas. O created_by é definido automaticamente.';

COMMENT ON POLICY "proposals_select_owner_or_admin" ON public.proposals IS 
'Permite que donos vejam suas propostas e admins vejam tudo. Propostas legacy (sem dono) são visíveis para todos.';

COMMENT ON POLICY "proposals_update_owner_or_admin" ON public.proposals IS 
'Permite que donos editem suas propostas e admins editem tudo. Protege contra alteração maliciosa do created_by.';

COMMENT ON POLICY "proposal_screens_select_proposal_owner" ON public.proposal_screens IS 
'Permite acesso às telas de proposta baseado nas permissões da proposta pai.';

COMMENT ON FUNCTION public.update_updated_at_column() IS 
'Função para atualizar automaticamente a coluna updated_at em triggers BEFORE UPDATE.';

COMMENT ON FUNCTION public.set_created_by() IS 
'Função para definir automaticamente created_by como auth.uid() em triggers BEFORE INSERT.';
