...
USING (
    ...
    EXISTS (
        ...
        WHERE p.id = email_logs.proposal_id 
...
CREATE OR REPLACE FUNCTION public.trigger_proposal_email_notifications()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
'Log de todos os emails enviados relacionados a propostas';

COMMENT ON FUNCTION public.create_email_log(BIGINT, VARCHAR(50), VARCHAR(255), VARCHAR(50), TEXT) IS 
'Cria um log de email para ser processado pelo sistema de envio';

COMMENT ON FUNCTION public.update_email_status(BIGINT, VARCHAR(20), TEXT) IS 
'Atualiza o status de um email (pending -> sent/failed)';

COMMENT ON FUNCTION public.trigger_proposal_email_notifications() IS 
'Trigger que cria logs de email quando propostas são criadas ou têm status alterado';

COMMENT ON VIEW public.email_stats IS 
