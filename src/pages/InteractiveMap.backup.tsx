...
// Importação dinâmica do Leaflet como fallback
...
export default function InteractiveMap() {
  ...
  useEffect(() => {
    if (mapboxToken && !map.current && mapContainer.current) {
      ...
      // Timeout para fallback se o token não chegar
      const timeoutId = setTimeout(() => {
        if (!map.current) {
          console.log('⏰ Timeout esperando token - usando fallback Leaflet...');
          initializeLeafletMap();
        }
      }, 5000); // 5 segundos para dar tempo ao Mapbox
  ...
  const fetchMapboxToken = async () => {
    try {
      ...
      setMapError('Erro ao carregar o mapa. Tente novamente mais tarde.');
    }
  };

  const initializeMap = () => {
    if (!mapContainer.current) {
      console.log('🚫 Container do mapa não disponível');
      return;
    }

    if (map.current) {
      console.log('🔄 Mapa já inicializado, removendo...');
      map.current.remove();
      map.current = null;
    }

    if (!mapboxToken) {
      console.log('⚠️ Token do Mapbox não disponível, tentando novamente...');
      fetchMapboxToken();
      return;
    }

    console.log('🗺️ Inicializando mapa com token:', mapboxToken.substring(0, 20) + '...');
    ...
    try {
      // Configurar Mapbox com validação
      console.log('🔍 Configurando Mapbox com token válido');
      ...
      console.error('💥 Erro ao criar mapa com Mapbox:', error);
      const errorMessage = error instanceof Error ? error.message : 'Erro desconhecido';
      
      // Tentar uma vez mais com Mapbox antes de usar Leaflet
      if (!mapboxToken.includes('fallback')) {
        console.log('🔄 Tentando novamente com token de fallback...');
        setTimeout(() => {
          try {
            initializeMap();
          } catch (retryError) {
            console.log('🔄 Fallback final com Leaflet...');
            initializeLeafletMap();
          }
        }, 1000);
      } else {
        console.log('🔄 Usando fallback Leaflet...');
        initializeLeafletMap();
      }
    }
  };

  // Função de fallback usando Leaflet
  const initializeLeafletMap = async () => {
    try {
      console.log('🍃 Inicializando mapa com Leaflet...');
      
      if (!mapContainer.current) {
        console.error('❌ Container não disponível para Leaflet');
        return;
      }
      ...
      // Criar mapa Leaflet com centro no Brasil
      ...
      // Simular interface do Mapbox para compatibilidade
      (leafletMap as any).flyTo = (options: any) => {
        leafletMap.flyTo([options.center[1], options.center[0]], options.zoom || 10);
      };

      (leafletMap as any).loaded = () => true;
      (leafletMap as any).isStyleLoaded = () => true;
      (leafletMap as any).remove = () => leafletMap.remove();
      ...
      // Adicionar marcadores se existirem telas
      if (filteredScreens.length > 0) {
        console.log('📍 Adicionando marcadores para', filteredScreens.length, 'telas');
        setTimeout(() => {
          updateLeafletMarkers(leafletMap, filteredScreens, L);
        }, 1000);
      }
  ...
  // Função para atualizar marcadores no Leaflet
  const updateLeafletMarkers = (leafletMap: any, screens: SimpleScreen[], L: any) => {
    console.log('🎯 Atualizando marcadores Leaflet para', screens.length, 'telas');
    
    // Limitar para mostrar apenas as primeiras 100 telas para performance
    const screensToShow = screens.slice(0, 100);
    ...
    screensToShow.forEach((screen, index) => {
      if (screen.lat && screen.lng) {
        ...
        if (isNaN(lat) || isNaN(lng)) {
          console.warn('❌ Coordenadas inválidas para tela', screen.code);
          return;
        }

        console.log(`📍 Adicionando marcador ${index + 1}: ${screen.code} em [${lat}, ${lng}]`);
        ...
        const customIcon = L.divIcon({
          ...
          html: `<div style="
            width: 24px; 
            height: 24px; 
            background: ${screen.active ? '#10b981' : '#6b7280'}; 
        ...
        marker.bindPopup(`
          <div style="padding: 12px; min-width: 250px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
            <h4 style="margin: 0 0 8px 0; font-weight: bold; color: #111827;">${screen.code} - ${screen.name}</h4>
            <div style="margin: 8px 0; padding: 8px; background: #f9fafb; border-radius: 6px;">
              <p style="margin: 2px 0; color: #374151; font-size: 12px;"><strong>Cidade:</strong> ${screen.city}, ${screen.state}</p>
              <p style="margin: 2px 0; color: #374151; font-size: 12px;"><strong>Classe:</strong> ${screen.class}</p>
              <p style="margin: 2px 0; color: #374151; font-size: 12px;"><strong>Status:</strong> ${screen.active ? '🟢 Ativa' : '🔴 Inativa'}</p>
            </div>
        ...
        marker.on('click', () => {
          console.log('🎯 Clique no marcador:', screen.code);
        });
      }
    });
    ...
    if (screensToShow.length > 0) {
      setTimeout(() => {
        const validScreens = screensToShow.filter(s => s.lat && s.lng && !isNaN(Number(s.lat)) && !isNaN(Number(s.lng)));
        if (validScreens.length > 0) {
          const group = L.featureGroup(validScreens.map(s => L.marker([Number(s.lat), Number(s.lng)])));
          leafletMap.fitBounds(group.getBounds().pad(0.1));
          console.log('✅ Zoom ajustado para mostrar', validScreens.length, 'marcadores');
        }
      }, 500);
    }
  };
