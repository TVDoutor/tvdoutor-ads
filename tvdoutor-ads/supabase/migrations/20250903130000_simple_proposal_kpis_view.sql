-- Create proposal_kpis view for dashboard statistics - apenas se tabelas existirem
-- This view consolidates proposal data for easy querying

-- Criar view apenas se as tabelas necessárias existirem
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'proposals' AND table_schema = 'public') THEN
        ...
        EXECUTE 'CREATE OR REPLACE VIEW public.proposal_kpis AS
        SELECT
            p.id,
            p.proposal_type,
            p.start_date,
